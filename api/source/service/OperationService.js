...
exports.getAppInfo = async function(options = {}) {
  const { includeRowCounts = true } = options
  ...
  // Set row counts from individual queries or use information_schema estimates
  if (includeRowCounts && rowCountResults) {
    ...
    // Use TABLE_ROWS from information_schema as estimates when not doing individual counts
    for (const tableName in tables) {
      tables[tableName].rowCount = tables[tableName].tableRows
    }
  }
