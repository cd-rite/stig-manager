...
describe('Collection put tests using "admin" user ', () => {
  ...
  describe('PUT - replaceCollection - /collections/{collectionId}', () => {
    ...
    it('Set all properties of a Collection', async () => {
        ...
        const res = await chai.request(config.baseUrl)
            .put(`/collections/${reviewEnv.testCollection.collectionId}`)
    ...
    it('Set all properties of a Collection- with metadata', async () => {
        ...
        const res = await chai.request(config.baseUrl)
            .put(`/collections/${reviewEnv.testCollection.collectionId}?elevate=true&projection=grants&projection=owners&projection=statistics&projection=stigs&projection=assets`)
            .set('Authorization', `Bearer ${user.token}`)
            .send(putRequest    )
        expect(res).to.have.status(200)
        ...
        expect(res.body.settings.status.minAcceptGrant).to.equal(putRequest.settings.status.minAcceptGrant)
        expect(res.body.settings.status.resetCriteria).to.equal(putRequest.settings.status.resetCriteria)
        expect(res.body.metadata.testkey).to.equal(reviewEnv.testCollection.metadataValue)
  ...
  describe('PUT - setStigAssetsByCollectionUser - /collections/{collectionId}/grants/{userId}/access', () => {
    ...
    it('set stig-asset grants for a lvl1 user in this collection.', async () => {
        ...
        const res = await chai.request(config.baseUrl)
            .put(`/collections/${reviewEnv.scrapCollection.collectionId}/grants/${reviewEnv.scrapLvl1User.userId}/access`)
            .set('Authorization', `Bearer ${user.token}`)
            .send([{
                  "benchmarkId": reviewEnv.scrapAsset.scrapBenchmark,
                  "assetId": reviewEnv.scrapAsset.assetId,
              }])
        expect(res).to.have.status(200)
        expect(res.body).to.have.lengthOf(1)
        for(const item of res.body){
            expect(item.benchmarkId).to.equal(reviewEnv.scrapAsset.scrapBenchmark)
            expect(item.asset.assetId).to.equal(reviewEnv.scrapAsset.assetId)
  ...
  describe('PUT - putCollectionMetadata - /collections/{collectionId}/metadata', () => {
    ...
    it('Set all metadata of a Collection', async () => {
        ...
        const res = await chai.request(config.baseUrl)
            .put(`/collections/${reviewEnv.testCollection.collectionId}/metadata`)
            .set('Authorization', `Bearer ${user.token}`)
            .send(putRequest)
        expect(res).to.have.status(200)
        expect(res.body[reviewEnv.testCollection.metadataKey]).to.equal(reviewEnv.testCollection.metadataValue)
    })
  })
  ...
  describe('PUT - putCollectionMetadataValue - /collections/{collectionId}/metadata/keys/{key}', () => {
    ...
    it('Set one metadata key/value of a Collection', async () => {
          const res = await chai.request(config.baseUrl)
              .put(`/collections/${reviewEnv.testCollection.collectionId}/metadata/keys/${reviewEnv.testCollection.metadataKey}`)
              .set('Authorization', `Bearer ${user.token}`)
