...
describe('Collection patch tests using "admin" user ', () => {
  ...
  describe('PATCH - updateCollection - /collections/{collectionId}', () => {
    ...
    it('Merge provided properties with a Collection', async () => {
        ...
        const res = await chai.request(config.baseUrl)
            .patch(`/collections/${reviewEnv.scrapCollection.collectionId}?elevate=true&projection=assets&projection=grants&projection=owners&projection=statistics&projection=stigs`)
        ...
        for(stig of res.body.stigs) {
            expect(stig.benchmarkId).to.be.oneOf(reviewEnv.scrapCollection.validStigs)
        }
    })
  })
  ...
  describe('PATCH - patchCollectionLabelById - /collections/{collectionId}/labels/{labelId}', () => {
    ...
    it('Merge provided properties with a Collection Label', async () => {
        const res = await chai.request(config.baseUrl)
            .patch(`/collections/${reviewEnv.testCollection.collectionId}/labels/${reviewEnv.testCollection.testLabel}`)
            .set('Authorization', `Bearer ${user.token}`)
            .send({
  ...
  describe('PATCH - patchCollectionMetadata - /collections/{collectionId}/metadata', () => {
    ...
    it('Merge metadata property/value into a Collection', async () => {
        ...
        const res = await chai.request(config.baseUrl)
            .patch(`/collections/${reviewEnv.testCollection.collectionId}/metadata`)
            .set('Authorization', `Bearer ${user.token}`)
            .send({[reviewEnv.testCollection.metadataKey]: reviewEnv.testCollection.metadataValue})
        
        expect(res).to.have.status(200)
        expect(res.body).to.contain({[reviewEnv.testCollection.metadataKey]: reviewEnv.testCollection.metadataValue})
    })
  })
})
