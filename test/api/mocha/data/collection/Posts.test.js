...
describe('Collection Post tests using "admin" user ', () => {
  ...
  describe("POST - cloneCollection - /collections/{collectionId}/clone", () => {
    ...
    before(async function () {
      this.timeout(4000)
      await utils.setDefaultRevision(reviewEnv.testCollection.collectionId, reviewEnv.testCollection.benchmark, "V1R0")
    })
    ...
    it("clone collection for later Review check and test projections everything matches source ", async () => {
      ...
      const res = await chai
        .request(config.baseUrl)
        .post(`/collections/${reviewEnv.testCollection.collectionId}/clone?projection=assets&projection=grants&projection=owners&projection=statistics&projection=stigs&projection=labels`        )
        .set("Authorization", `Bearer ${user.token}`)
        .send({
        ...
        const sourceCollectionReviews = await utils.getReviews(reviewEnv.testCollection.collectionId)
        ...
        const sourceCollection = await utils.getCollection(reviewEnv.testCollection.collectionId)
    ...
    const ownersProjected = [
      ...
      const res = await chai
      .request(config.baseUrl)
      .post(`/collections/${reviewEnv.testCollection.collectionId}/clone?projection=grants&projection=owners`        )
      .set("Authorization", `Bearer ${user.token}`)
      .send({
        name:"Clone_" + Math.floor(Math.random() * 100) + "-" + Math.floor(Math.random() * 100) + "_X",
        description: "clone of test collection x",
    ...
    it("clone test collection - no labels", async () => {
      ...
      const res = await chai
      .request(config.baseUrl)
      .post(`/collections/${reviewEnv.testCollection.collectionId}/clone?projection=labels`        )
      .set("Authorization", `Bearer ${user.token}`)
      .send({
        name:"Clone_" + Math.floor(Math.random() * 100) + "-" + Math.floor(Math.random() * 100) + "_X",
        description: "clone of test collection x",
    ...
    it("clone test collection - no assets", async () => {
      ...
      const res = await chai
      .request(config.baseUrl)
      .post(`/collections/${reviewEnv.testCollection.collectionId}/clone?projection=assets`        )
      .set("Authorization", `Bearer ${user.token}`)
      .send({
        name:"Clone_" + Math.floor(Math.random() * 100) + "-" + Math.floor(Math.random() * 100) + "_X",
        description: "clone of test collection x",
    ...
    it("clone test collection - stigMappings=none", async () => {
      ...
      const res = await chai
      .request(config.baseUrl)
      .post(`/collections/${reviewEnv.testCollection.collectionId}/clone?projection=statistics&projection=stigs`)
      .set("Authorization", `Bearer ${user.token}`)
      .send({
        name:"Clone_" + Math.floor(Math.random() * 100) + "-" + Math.floor(Math.random() * 100) + "_X",
        description: "clone of test collection x",
  ...
  describe("POST - exportToCollection - /collections/{collectionId}/export-to/{dstCollectionId}", () => {
    ...
    it("export results to another collection - entire asset - create asset in destination", async () => {
      ...
      const res = await chai
        .request(config.baseUrl)
        .post(`/collections/${reviewEnv.testCollection.collectionId}/export-to/${reviewEnv.scrapCollection.collectionId}`)
        .set("Authorization", `Bearer ${user.token}`)
        .send([
          {
            assetId: reviewEnv.testAsset.assetId,
          },
        ])
        expect(res).to.have.status(200)
        const response = res.body.toString().split("\n")
  ...
  describe("POST - createCollectionLabel - /collections/{collectionId}/labels", () => {
    ...
    it("Create Label in a Collection", async () => {
      ...
      const res = await chai
        .request(config.baseUrl)
        .post(`/collections/${reviewEnv.scrapCollection.collectionId}/labels`)
        .set("Authorization", `Bearer ${user.token}`)
        .send(request)
        expect(res).to.have.status(201)
  ...
  describe("POST - writeStigPropsByCollectionStig - /collections/{collectionId}/stigs/{benchmarkId}", () => {
    before(async function () {
      this.timeout(4000)
      await utils.loadAppData()
      await utils.uploadTestStigs()
    ...
    it("Set the Assets mapped to a STIG - default rev and assets", async () => {
      ...
      const res = await chai
        .request(config.baseUrl)
        .post(`/collections/${reviewEnv.testCollection.collectionId}/stigs/${reviewEnv.testCollection.benchmark}`)
        .set("Authorization", `Bearer ${user.token}`)
        .send(post)
        expect(res).to.have.status(200)
