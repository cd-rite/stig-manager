...
describe('Review delete tests using "admin" user ', () => {
  ...
  describe('DELETE - deleteReviewByAssetRule - /collections/{collectionId}/reviews/{assetId}/{ruleId}', () => {
    ...
    it('Delete a Review - freshRuleId - review may or may not exist', async () => {
      const res = await chai.request(config.baseUrl)
        .delete(`/collections/${reviewEnv.testCollection.collectionId}/reviews/${reviewEnv.testAsset.assetId}/${reviewEnv.freshRuleId}?projection=rule&projection=history&projection=stigs`)
        .set('Authorization', `Bearer ${user.token}`)
